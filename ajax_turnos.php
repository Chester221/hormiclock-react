...
try {
    switch ($accion) {
        ...
        case 'eliminar_pasados':
            // Eliminar turnos con fecha anterior a hoy
            $hoy = date('Y-m-d');
        ...
        case 'actualizar_estados':
            // Función para actualizar automáticamente estados de turnos
            $hoy = date('Y-m-d');
            $ahora = date('H:i:s');
            
            // Turnos vencidos (fecha pasada)
            $stmt = $conn->prepare("UPDATE turnos SET estado = 'vencido' WHERE user_id = ? AND fecha < ? AND estado NOT IN ('completado', 'confirmado')");
            $stmt->execute([$user_id, $hoy]);
            
            // Turnos completados (fecha pasada Y hora fin ya pasó)
            $stmt = $conn->prepare("UPDATE turnos SET estado = 'completado' WHERE user_id = ? AND fecha = ? AND hora_fin < ? AND estado = 'confirmado'");
            $stmt->execute([$user_id, $hoy, $ahora]);
